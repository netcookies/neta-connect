package com.neta.isulewtools.api.widget.icons
...
/**
 * Material Icons 图标提供器
 * 用于根据图标名称获取对应的 ImageVector
 */
object MaterialIconsProvider {
    private val iconsCache: List<Pair<String, ImageVector>> by lazy {
        listOf(
            // 常用操作
            ...
            "CloudUpload" to Icons.Default.CloudUpload,
            "CloudDownload" to Icons.Default.CloudDownload,
            ...
            "RemoveCircle" to Icons.Default.RemoveCircle,
     ...
     * 获取所有图标列表
     */
    fun getAllIcons(): List<Pair<String, ImageVector>> = iconsCache

    /**
     * 根据名称获取图标
     * @param name 图标名称
     * @return 对应的 ImageVector，如果未找到则返回 null
     */
    fun getIconByName(name: String): ImageVector? {
        return iconsCache.firstOrNull { it.first == name }?.second
    }
}
